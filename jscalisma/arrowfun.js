////// Arrow Functions

// ES5

let welcomeES5 = function () {
  // console.log("Hello from ES5");
};

welcomeES5();

//ES6
let welcomeES6 = () => {
  // console.log("Hello from ES6");
};

welcomeES6();

// With parameters

// ES5

let multiplyES5 = function (x, y) {
  return x * y;
};

// console.log(multiplyES5(10, 5));

//ES6

let multiplyES6 = (x, y) => x * y;
// console.log(multiplyES6(5, 6));

//ES5

let split = function (text) {
  return text.split(" ");
};

// console.log(split("Eren Ertürk"));

//ES6

let splitES6 = (text) => text.split("  ");

// console.log(splitES6("AAAA BBBB CCCC "));

/// Object Literals

//ES5

let getProductES5 = function (id, name) {
  return {
    id: id,
    name: name,
  };
};

// console.log(getProductES5("1", "Eren"));

//ES6

let getProductES6 = (id, name) => ({
  id: id,
  name: name,
});

// console.log(getProductES6("2", "CAN"));

let phones = [
  { name: "Iphone", price: 70000 },
  { name: "Samsung", price: 50000 },
  { name: "Huawei", price: 40000 },
  { name: "Honor", price: 20000 },
];

//ES5
let pricesES5 = phones.map(function (phone) {
  return phone.price;
});

// console.log(pricesES5);

//ES6

let pricesES6 = phones.map((phone) => phone.price);
// console.log(pricesES6);

//ES5 FILTER
let arr = [1, 2, 3, 4, 5, 50, 55, 66, 21, 33, 74, 27, 46, 33];
let evenES5 = arr.filter(function (a) {
  return a % 2 == 1;
});

// console.log(evenES5);

//ES6 FILTER

let evenES6 = arr.filter((a) => a % 2 == 1);
// console.log(evenES6);
